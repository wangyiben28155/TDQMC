...
using Distributions                   #用来初始化初始的波函数和系综粒子分布
import Base.@kwdef

const Ensemble_num, L, x_num, step_t = (50, 350.0, 1751, 13001)


@kwdef mutable struct Dynamics{T<:AbstractFloat}
    Guide_Wave::Vector{Complex{T}} = complex(collect(pdf(Truncated(Normal(0, 30), -Inf64, Inf64), LinRange(-L, L, x_num))))
