...
V_ee(x_t::T; x::AbstractVector{T}, β::T = 0.2) where {T<:AbstractFloat} = abs(x_t) == Inf ? zeros(typeof(x_t), length(x)) : 
                                                                                            @. 1.0 / sqrt(β + (x - x_t)^2)
...
function Vtd_Operator(x_t::Vector{T}, P::Parameter, t::Union{T,Complex{T}};
    ...
    if imag(P.Δt) == 0
        Matrix_td .+= repeat(Field(t) .* P.sampling, outer = (1, P.electron))
    end
