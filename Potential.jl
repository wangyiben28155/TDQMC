...
using ..TDQMC
import ..TDQMC:Electron_num, a, b
using SparseArrays, LinearAlgebra

#import PhysicalConstants.CODATA2018:e, a_0, ħ, m_e, c_0

#const ω_unit = ħ / (m_e * a_0^2)
#const E_unit = e/(4pi*ε_0*a_0^2)
#const λ = 800E-9    # 800nm波长
#const ω_0 = round(((2pi * c_0 / λ) / ω_unit).val, digits = 3) #取小数点后3位
#const ξ_0 = round(sqrt(I)*1e2/sqrt(ε_0*c_0*E_unit^2/2), digits = 3)
const λ = 800   # 800nm波长
const I₀ = 3e14
const ω_0 = round(45.56 / λ, digits=3)
const T_0 = 2pi / ω_0
const ξ_0 = round(sqrt(I₀) * 5.338e-9, digits=3)
const n_c = 20

# 导波函数部分
V_ne(x::T; α::T = a) where {T<:AbstractFloat} = -float(Electron_num) / sqrt(α + x^2)                         #定义势能项, 因为这里不需要对矩阵进行运算, 就不需要用
Envelope(t::T; ξ_0::T=ξ_0, ω::T=ω_0, T_0::T=T_0) where {T<:AbstractFloat} = t<=(n_c*T_0) ? sin(ω*t/(2n_c))^2 : 0.0
Electric_Field(t::T; ϵ::Function=Envelope, ω::T=ω_0) where {T<:AbstractFloat} = ϵ(t) * sin(ω * t)   #定义电场

V_ee(x_t::T; x::AbstractVector{T}, β::T = b) where {T<:AbstractFloat} = @. 1.0 / sqrt(β + (x - x_t)^2)
